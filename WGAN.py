...
def discriminator(D_input, reuse = False):
    ...
    output = tf.matmul(hidden, D_FW1) + D_Fb1
    return output
...
d_optimizer = tf.train.RMSPropOptimizer(learning_rate=0.0002).minimize(d_loss, var_list=d_var_list)
g_optimizer = tf.train.RMSPropOptimizer(learning_rate=0.0002).minimize(g_loss, var_list=g_var_list)
clip_D = [p.assign(tf.clip_by_value(p, -0.01, 0.01)) for p in d_var_list]
...
for epoch in range(epochs):
    ...
    while batch_i < len(train_images):
        ...
        _, loss_val_D, _ = sess.run([d_optimizer, d_loss, clip_D], feed_dict={X: batch_X, Z: batch_Z})
        _, loss_val_G = sess.run([g_optimizer, g_loss], feed_dict={Z: batch_Z})
        batch_i += batch_size
...
#         if batch_i >= len(train_images):
#             batch_i = 0
#     for _ in range(2):
#         #i_Z = np.random.uniform(-1., 1., size=[batch_size, n_noise])
#         _, loss_val_G = sess.run([g_optimizer, g_loss], feed_dict={Z: batch_Z})
    
#     print('iteration {:3} D_Loss: {:>6.3f} G_Loss: {:>6.4f} Epoch duration: {:>6.3f}s'.format(i, loss_val_D, loss_val_G, time.time() - start_time))
